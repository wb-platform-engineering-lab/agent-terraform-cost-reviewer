...
def parse_report(text: str) -> dict:
    ...
    for line in text.split("\n"):
        ...
        if last_check_idx is not None and re.search(r"est\.?\s*sav", s, re.IGNORECASE):
            ...
            # Also store the raw saving amount for aggregation
    ...
    # ── Aggregate savings across all failing checks ───────────────────
...
def _aggregate_savings(text: str, checks: list) -> dict:
    """
    Extract and aggregate estimated savings from the report.
    Returns {"low": N, "high": N, "label": "$X–Y/mo", "count": N}
    where low/high are monthly $ figures.
    """
    # Collect all dollar-range patterns from the full text
    # Matches: ~$14/mo, $100–500/mo, ~$20-300/mo, $3.65/mo
    pattern = re.compile(
    ...
    for m in pattern.finditer(text):
        # Deduplicate by rounded position to avoid counting the same figure twice
        ...
        # Sanity: skip tiny amounts (<1) and huge outliers (>50k/mo)
    ...
    if abs(high_total - low_total) < 5:
        label = f"{_fmt(low_total)}/mo"
    else:
        label = f"{_fmt(low_total)}–{_fmt(high_total)}/mo"
        ...
        <div class="flex-shrink-0 flex flex-col sm:flex-row gap-4">
          ...
          <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-6 text-center min-w-[140px]">
            ...
            <div class="text-emerald-500 text-xs mt-2 flex items-center justify-center gap-1">
              ...
              <span>{savings_count} issue{'s' if savings_count != 1 else ''} found</span>
            </div>
          </div>
        </div>
      </div>
